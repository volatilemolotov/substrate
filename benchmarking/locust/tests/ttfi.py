...
LOCUST_NO_TTFI_USER=1 to skip loading this file, leaving boomer as the sole
owner of TTFIUser load.

TTFI must be measured from Go: the window is a wall clock, so a gevent
scheduling delay between the response arriving and the greenlet resuming
would land in the number itself rather than beside it.
"""

import os

if os.environ.get("LOCUST_NO_TTFI_USER") != "1":
    ...
    # runtime flag values (trace probability, wait times, TTFI config) the
    ...
    class TTFIUser(User):
        ...
        def noop(self) -> None:
            ...
            # does not load this file (LOCUST_NO_TTFI_USER=1). Body is
